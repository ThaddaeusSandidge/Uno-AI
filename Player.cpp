...
void Player::recreateAI()
{
	...
	case 1:
		...
		//... imagine there are 19 more rows here for yall's AIs
	case 2:
		theAI = new UDK1(playerNumber);
		break;
	case 3:
		theAI = new UDK2(playerNumber);
		break;
	case 4:
		theAI = new UDK3(playerNumber);
		break;
	case 5:
		theAI = new UDK4(playerNumber);
		break;
	case 6:
		theAI = new UDK5(playerNumber);
		break;
	case 7:
		theAI = new UDK6(playerNumber);
		break;
	case 8:
		theAI = new UDK7(playerNumber);
		break;
...
int Player::makeMove(Card justPlayed, Color choosenCardColor, bool justDrew, vector<int> scores, vector<int> cardAmountsByPlayer,int direction) //the returned int refers to the index of which card is played in your hand.
{
	...
	if (!validPlay(index, justPlayed, choosenCardColor, justDrew, scores, cardAmountsByPlayer, hand) && index != -1)
	{
