...
class UDK5 : public AI
{

private:
	Color savedColor = RED;
	vector<int> colorPriority;
	vector<vector<Card>> colorVec;
	int firstC;
	int secondC;
	int thirdC;
	int fourthC;
	int numOfColorsInHand;
	//vector<int> allCardCount;

public:
	UDK5(int playerNumber);
	...
	bool playFirstC(vector<Card> playableCards);
	bool playSecondC(vector<Card> playableCards);
	bool playThirdC(vector<Card> playableCards);
	bool playFourthC(vector<Card> playableCards);
	void colorPrioritySort(vector<Card> cardsInHand);
